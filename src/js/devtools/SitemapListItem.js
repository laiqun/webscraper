...
let SitemapListItem = class extends react.Component {
	...
	editSitemap(e) {
		e.stopPropagation();
		e.preventDefault();
		this.props.appState.push("/sitemap/selectors?sitemapId=" + this.props.sitemap.name);
	}

	async deleteSitemap(e) {
		e.stopPropagation();
		e.preventDefault();
		await this.props.appState.deleteSitemap(this.props.sitemap.name);
	}

	async downloadSitemap(e) {
		e.stopPropagation();
		e.preventDefault();
		await this.props.appState.downloadSitemap(this.props.sitemap.name);
	...
	async uploadSitemap(e) {
		e.stopPropagation();
		e.preventDefault();
		await this.props.appState.uploadSitemap(this.props.sitemap.name);
		await this.props.appState.loadSitemaps();
	}
